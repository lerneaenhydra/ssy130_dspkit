...
//HP filter coefficients set during runtime
float hpfilt_coeffs[TEST5_FILTLEN] = {0};
float hpfilt_state[NUMEL(hpfilt_coeffs) + AUDIO_BLOCKSIZE - 1];
arm_fir_instance_f32 hpfilt_s;
bool hpfilt_enbl = true;

void example5_helptext(void){
	printf(	"\nPress:\n"
			"\t1 - Increase modulation frequency\n"
			"\t2 - Decrease modulation frequency\n"
			"\t3 - Toggle highpass filter active\n");
}
...
void example_test5_init(void){
	...
	example5_helptext();
	arm_fir_init_f32(&hpfilt_s, NUMEL(hpfilt_coeffs), hpfilt_coeffs, hpfilt_state, AUDIO_BLOCKSIZE);
}
...
void example_test5(void){
	...
	if(newfreq){
		...
		printf("Modulation frequency updated to %g [Hz]\n", sinfreq);
		blocks_sources_trig_setfreq(sinfreq);

		//Update highpass filter

		//Generate reference lowpass filter
		float sinc[TEST5_FILTLEN];
		const float relfreq = (sinfreq + TEST5_BASE_CUTOFF_Hz)/AUDIO_SAMPLE_RATE;

		blocks_gen_sinc(relfreq, sinc, NUMEL(sinc));
		//Scale to give correct amplitude response
		arm_scale_f32(sinc, 2*relfreq, sinc, NUMEL(sinc));
		...
		arm_sub_f32(sinc, ap, hp, NUMEL(hp));
		...
		arm_mult_f32(hp, window, hpfilt_coeffs, NUMEL(hpfilt_coeffs));
	}
        
	//Get source samples
	blocks_sources_microphone(inpdata);
	blocks_sinks_rightout(inpdata);	//Send input data directly to right speaker

	//Apply frequency shift
	float carrier[AUDIO_BLOCKSIZE];
	blocks_sources_sin(carrier);

	arm_mult_f32(inpdata, carrier, processed_data, NUMEL(processed_data));

	//Optionally apply highpass filter
	if(hpfilt_enbl){
		arm_fir_f32(&hpfilt_s, processed_data, filt_data, AUDIO_BLOCKSIZE);
	}else{
